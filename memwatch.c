...
void mwInit( void ) {
    ...
    /* start a log if none is running */
        if( mwLogR() == NULL ) mwLogFile( "\\\\psf\\Home\\Documents\\Programmering\\DSTII IAR\\memwatch.log" );
    if( mwLogR() == NULL ) {
        int i;
        char buf[32];
...
void mwAbort( void ) {
    ...
    while( mwHead != NULL && errors < 3 ) {
        ...
        if( !(mwHead->flag & MW_NML) ) {
            ...
            if( mwTestMem( data, (unsigned int)mwHead->size, MW_VAL_NML ) ) {
                mwErrors++;
...
void * mwMark( void *p, const char *desc, const char *file, unsigned line ) {
    ...
    if( mrk == NULL ) {
        ...
        isnew = 0;
        n = (unsigned int)strlen( mrk->text );
        }
...
static size_t mwFreeUp( size_t needed, int urgent ) {
    ...
    while( mw != NULL ) {
        if( !(mw->flag & MW_NML) ) mw = mw->next;
            ...
            if( mwTestMem( data, (unsigned int)mw->size, MW_VAL_NML ) ) {
                mwIncErr();
                mwWrite( "wild pointer: <%ld> NoMansLand %p alloc'd at %s(%d)\n",
...
static void mwStatReport()
{
    ...
    for( ms=mwStatList; ms; ms=ms->next )
    {
        if( ms->line == -1 )
        {
			...
			modnamelen = (unsigned int)strlen(modname);
			if( modnamelen > 42 )
			{
